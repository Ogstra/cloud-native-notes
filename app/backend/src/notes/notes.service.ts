...
export class NotesService {
    ...
    async create(userId: number, dto: CreateNoteDto) {
        ...
        const first = await this.prisma.note.findFirst({
            where: { userId },
    ...
    async reorder(notes: { id: number; position: number }[]) {
    ...
    async seedDemoData(userId: number) {
        ...
        const createdNotes = await this.prisma.note.findMany({
            where: { userId },
            orderBy: { id: 'asc' },
